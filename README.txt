...
- フレーム間隔設定（1-10フレームごとの解析、スローモーション対応）
- ズーム・ドラッグによる動画拡大・移動
- 手動トラッキング（クリックで物体位置を記録、マーカー表示、1物体/2物体対応）
- 記録データのCSV出力（秒単位、ヘッダー付き、小数点以下3桁、座標系・符号・順序は画面表示と一致）
- Undo機能：追跡モード中はガイドテキスト横にUndoボタン＋Zキー/BackspaceでもUndo可能（フレーム間隔対応）
- UIは日本語対応
...
4. フレームレート・解析範囲・フレーム間隔を必要に応じて設定
5. ズーム（＋−リセット）やドラッグで動画を見やすく調整
6. 「手動追跡モード」ON→物体の位置をクリック（全物体分クリック後にNフレーム進む）
7. 追跡モード中はガイド横のUndoボタンまたはZ/Backspaceで直前の点を取り消し（フレーム間隔対応）
...
- フレーム間隔機能追加（1-10フレームごとの解析、スローモーション撮影対応）
...
■ フレーム間隔機能について
- スローモーション撮影（240fps等）でも効率的な解析が可能
- フレーム間隔=1：従来通り1フレームごと
- フレーム間隔=5：5フレームごと（フレーム0, 5, 10, 15...）
- フレーム間隔=10：10フレームごと（フレーム0, 10, 20, 30...）
- Undo時もフレーム間隔に応じて適切に戻る
